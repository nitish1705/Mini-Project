...
-----
  ...
  demo      Quick demonstration (few episodes) with visualisatin.
...
def run_saved_model_inference(
    ...
    intrinsic = IntrinsicController(
        ...
        gamma=config.get("gamma", 0.99),
        max_hops=config.get("max_hops", 15),
    )
...
def write_results_note(save_dir: str):
    """Write a short provenance note for generated results artifacts."""
    os.makedirs(save_dir, exist_ok=True)
    note_path = os.path.join(save_dir, "simulator_note.txt")
    with open(note_path, "w") as f:
        f.write(
            "This results set was generated by the Python FANET simulator.\n"
            "It validates routing logic and integration behavior.\n"
            "Full ns-3/ns3-gym PHY-MAC fidelity is future work.\n"
        )
...
def parse_args():
    ...
    p.add_argument(
        ...
        type=float,
        default=250.0,
        help="UAV communication range in meters",
    )
    p.add_argument(
        "--max_speed",
        type=float,
        default=15.0,
        help="Maximum UAV speed in m/s",
    )
    p.add_argument(
        "--max_hops",
        type=int,
...
def main():
    ...
    if args.mode in ("train", "demo"):
        ...
        write_results_note(args.save_dir)
